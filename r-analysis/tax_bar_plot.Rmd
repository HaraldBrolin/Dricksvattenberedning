...
```{r prune and glom}
# df <- prune_samples(df@sam_data$location == "overby", df)
...
reads_for_phylum <- 
  function(df_level, phylum_tag) {
    asv_index <- (tax_table(df_level)[,2] == phylum_tag) %>% which() # Ge index for each asv belonging to phylum_tag
    reads_per_asv <- df_level@otu_table[asv_index,] %>% rowSums() # 
    phylum_sum <- reads_per_asv %>% sum
    total_read_sum <- df_level %>% taxa_sums() %>% sum()
    phylum_tag_relative_abundence <- phylum_sum / total_read_sum
}
