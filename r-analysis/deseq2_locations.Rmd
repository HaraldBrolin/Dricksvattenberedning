...
df_ost@sam_data$group[which(df_ost@sam_data$location == "Härnösand")] <- "extreme"
df_ost@sam_data$group[which(df_ost@sam_data$location != "Härnösand")] <- "group"

# df@sam_data <- df@sam_data %>% rownames_to_column %>% mutate(group = case_when(location == "Härnösand" ~ 0, location != "Härnösand" ~ 1 )) 
```
...
deseq_2_and_result <- function(phylo, rarefy, betaPrior, alpha, dispersionPlot){
  ...
  return(list(significant_table, result_plot)) # Return the final table but also the DESeq outpt, since it is needed for a plot
}
...
deseq_1 <- deseq_2_and_result(df_ost, rarefy = TRUE, betaPrior = TRUE, alpha = 0.001, dispersionPlot = TRUE)
deseq_2 <- deseq_2_and_result(df_ost, rarefy = FALSE, betaPrior = TRUE, alpha = 0.001, dispersionPlot = FALSE)
...
asv_scatter_plot$in_model[(asv_scatter_plot %>% rownames()) %in% (deseq_1[[2]] %>% rownames())] <- 
  "Significant"
...
df_harno@sam_data$group[which(df_harno@sam_data$location == "Östersund")] <- "extreme"
df_harno@sam_data$group[which(df_harno@sam_data$location != "Östersund")] <- "group"
...
deseq_3 <- deseq_2_and_result(df_harno, rarefy = TRUE, betaPrior = TRUE, alpha = 0.001, dispersionPlot = TRUE)
deseq_4 <- deseq_2_and_result(df_harno, rarefy = FALSE, betaPrior = TRUE, alpha = 0.001, dispersionPlot = FALSE)
...
deseq_4[[2]] # Scatter plot of significant ASVs

df_plot <- deseq_4[[1]] %>% rownames_to_column() # To generate a ggplot df
...
asv_scatter_plot$in_model[(asv_scatter_plot %>% rownames()) %in% (deseq_3[[2]] %>% rownames())] <- 
  "Significant"
...
#### Conclusion:
Rarefying seems to increase the number of significant ASVs, this is agains my presumptiom since rarefying should reduce FDR. 
